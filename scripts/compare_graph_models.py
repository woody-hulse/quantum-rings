...
This script trains and evaluates all GNN architectures on the quantum circuit
threshold prediction task, providing detailed metrics for comparison.

Models compared:
- BasicGNN: Simple message-passing GNN
- ImprovedGNN: Attention-based with ordinal regression
...
class AggregatedResult:
    ...
    val_score_std: float
    underpred_rate_mean: float
    overpred_rate_mean: float
...
def aggregate_results(results: List[ModelResult], model_type: str) -> AggregatedResult:
...
def print_comparison_table(aggregated: List[AggregatedResult]) -> None:
...
def run_comparison(
    ...
    print("=" * 80)
    print("QUANTUM CIRCUIT GRAPH MODEL COMPARISON")
    print("=" * 80)
    ...
    aggregated = []
    for model_type in model_types:
        agg = aggregate_results(all_results, model_type)
        if agg:
...
def main():
    ...
    if args.output:
        ...
        output_file = project_root / "results" / f"graph_model_comparison_{timestamp}.json"
