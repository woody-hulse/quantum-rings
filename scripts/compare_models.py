...
def main():
    ...
    parser.add_argument("--models", type=str, default="mlp,catboost,lightgbm",
    ...
    if args.include_ensemble and len(models) >= 2:
        ...
        cmd = [
            ...
            str(evaluate_script),
            "--model", "ensemble",
            "--ensemble-models", ",".join(models[:2]),  # Use top 2 models
            "--n-runs", str(min(args.n_runs, 10)),  # Cap at 10 for ensemble
            "--epochs", str(args.epochs),
            "--batch-size", str(args.batch_size),
            "--seed", str(args.seed),
            "--device", args.device,
