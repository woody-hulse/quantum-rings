"""
Rohan MLP: Neural network model with correlation-based feature selection.

MLP (Multi-Layer Perceptron) with:
- Correlation-based feature selection (same as rohan7/rohan8)
- Separate feature sets for threshold and runtime prediction
- Multi-task learning with shared hidden layers

Top Features (same as rohan7 and rohan8):
...
class RohanMLPNetwork(nn.Module):
    """Multi-task MLP with shared encoder for threshold and runtime prediction."""
...
class RohanMLPModel(BaseModel):
    """MLP model with correlation-based feature selection."""
    
    # Top features for each task (based on correlation analysis - same as rohan7/rohan8)
    THRESHOLD_FEATURES = [
        ...
        'final_light_cone_size', 'entanglement_velocity', 'avg_span'
    ]
    ...
    RUNTIME_FEATURES = [
        ...
        'cut_crossing_ratio', 'nearest_neighbor_ratio'
    ]
    ...
    def __init__(
        ...
        # Network will be resized after feature selection
        self.network: Optional[RohanMLPNetwork] = None
        self.selected_input_dim = input_dim
    ...
    def _extract_data(self, loader: DataLoader) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        for batch in loader:
            ...
            all_thresh.append(batch["threshold_class"].numpy())
            all_runtime.append(batch["log_runtime"].numpy())
        
        X = np.vstack(all_features)
        y_thresh = np.concatenate(all_thresh)
        y_runtime = np.concatenate(all_runtime)
    ...
    def _select_features(self, X: np.ndarray, y: np.ndarray, feature_names: List[str], 
        ...
        for feat_name in target_features:
            if feat_name in feature_names:
                selected_indices.append(feature_names.index(feat_name))
    ...
    def _train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch in train_loader:
            ...
            # Select only the relevant features (union of both feature sets)
            features = features[:, self.all_selected_indices]
    ...
    def fit(
        ...
        # Extract data and select features
        ...
        self.threshold_feature_indices = self._select_features(X_train, y_thresh_train, feature_names, self.THRESHOLD_FEATURES)
        self.runtime_feature_indices = self._select_features(X_train, y_runtime_train, feature_names, self.RUNTIME_FEATURES)
        
        # Union of features used (same strategy as rohan7/rohan8)
        self.all_selected_indices = sorted(set(self.threshold_feature_indices + self.runtime_feature_indices))
        self.selected_input_dim = len(self.all_selected_indices)
        
        if verbose:
            print(f"Selected {len(self.threshold_feature_indices)} threshold features")
            print(f"Selected {len(self.runtime_feature_indices)} runtime features")
            print(f"Total unique features: {self.selected_input_dim}")
        
        # Create network with selected feature dimension
        self.network = RohanMLPNetwork(
            input_dim=self.selected_input_dim,
    ...
    def save(self, path: Path) -> None:
        ...
        torch.save({
            ...
            "threshold_feature_indices": self.threshold_feature_indices,
            "runtime_feature_indices": self.runtime_feature_indices,
            "all_selected_indices": self.all_selected_indices,
    ...
    def load(self, path: Path) -> None:
        ...
        self.threshold_feature_indices = checkpoint["threshold_feature_indices"]
        self.runtime_feature_indices = checkpoint["runtime_feature_indices"]
        self.all_selected_indices = checkpoint["all_selected_indices"]
        self.selected_input_dim = len(self.all_selected_indices)
